...
//            System.out.println("# threads: "+ ForkJoinPool.commonPool().getRunningThreadCount());
    ...
    public static void sort(int[] array, int from, int to, ForkJoinPool customThreadPool) {
        if (to - from < cutoff) Arrays.sort(array, from, to);
        else {
            CompletableFuture<int[]> parsort1 = parsort(array, from, from + (to - from) / 2, customThreadPool);
            CompletableFuture<int[]> parsort2 = parsort(array, from + (to - from) / 2, to, customThreadPool);
            CompletableFuture<int[]> parsort = parsort1.thenCombine(parsort2, (xs1, xs2) -> {
                int[] result = new int[xs1.length + xs2.length];
                int i = 0, j = 0;
                for (int k = 0; k < result.length; k++) {
    ...
    private static CompletableFuture<int[]> parsort(int[] array, int from, int to, ForkJoinPool customThreadPool) {
        return CompletableFuture.supplyAsync(
                () -> {
                    int[] result = new int[to - from];
                    System.arraycopy(array, from, result, 0, result.length);
                    sort(result, 0, to - from, customThreadPool);
                    return result;
                },
                customThreadPool 
        );
